    ...
    <NavigationMenu>
      <NavigationMenuList>
        <NavigationMenuItem className="bg-transparent">
          <NavigationMenuTrigger
            className="link link-hover text-base-content/80 hover:text-base-content active:text-base-content focus:text-base-content duration-100 text-base"
            style={{ backgroundColor: "transparent", padding: "0px" }}
          >
            Categories
          </NavigationMenuTrigger>
          <NavigationMenuContent>
            <ul className="grid w-[250px] sm:w-[335px] gap-3 p-4">
              {categories.map((category) => (
                <ListItem
                  key={category.slug}
                  title={category?.titleShort || category.title}
                  href={`/blog/category/${category.slug}`}
                >
    ...
    <header className="bg-base-200">
      <nav className="max-w-7xl flex items-center justify-between px-8 py-3 mx-auto">
        ...
        <Sheet>
          ...
          <SheetContent className="lg:hidden min-w-[320px]">
            <SheetHeader>
        ...
        <div className="hidden lg:flex lg:justify-center lg:gap-12 lg:items-center">
          {links.map((link) => (
            <Link
              href={link.href}
              key={link.href}
              className="link link-hover text-base-content/80 hover:text-base-content active:text-base-content focus:text-base-content duration-100 font-medium"
              title={link.label}
            >
              {link.label}
            </Link>
